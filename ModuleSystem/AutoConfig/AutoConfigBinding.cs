...
namespace ThinkInvisible.Admiral {
    public class AutoConfigBinding {
        ...
        internal static void CleanupDirty(bool isRunEnd) {
            AdmiralPlugin._logger.LogDebug($"Stage ended; applying {stageDirtyInstances.Count} deferred config changes...");
            ...
            if(isRunEnd) {
                AdmiralPlugin._logger.LogDebug($"Run ended; applying {runDirtyInstances.Count} deferred config changes...");
        ...
        internal void UpdateProperty(object newValue, bool silent = false) {
            if(deferType == DeferType.UpdateImmediately || Run.instance == null || !Run.instance.enabled) {
                ...
                AdmiralPlugin._logger.LogWarning($"Something attempted to set the value of an AutoConfigBinding with the DeferForever flag: \"{readablePath}\"");
            }
        }
