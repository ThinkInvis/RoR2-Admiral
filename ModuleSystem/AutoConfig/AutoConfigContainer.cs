    ...
    public class AutoConfigContainer {
                ...
                switch(strParams[1]) {
                    case "Prop":
                        if(strParams.Length < 3){
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(not enough params for Prop tag).");
                            return m.Value;
                        }
                        var iprop = prop.DeclaringType.GetProperty(strParams[2], BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                        if(iprop == null) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(could not find Prop \"{strParams[2]}\").");
                            return m.Value;
                        }
                        return iprop.GetValue(this).ToString();
                    case "Field":
                        if(strParams.Length < 3) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(not enough params for Field tag).");
                            return m.Value;
                        }
                        var ifld = prop.DeclaringType.GetField(strParams[2], BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                        if(ifld == null) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(could not find Field \"{strParams[2]}\").");
                            return m.Value;
                        }
                        return ifld.GetValue(this).ToString();
                    case "DictKey":
                        if(!subDict.HasValue) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(DictKey tag used on non-BindDict).");
                            return m.Value;
                        }
                        return subDict.Value.key.ToString();
                    case "DictInd":
                        if(!subDict.HasValue) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(DictInd tag used on non-BindDict).");
                            return m.Value;
                        }
                        return subDict.Value.index.ToString();
                    case "DictKeyProp":
                        if(!subDict.HasValue) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(DictKeyProp tag used on non-BindDict).");
                            return m.Value;
                        }
                        if(strParams.Length < 3){
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(not enough params for DictKeyProp tag).");
                            return m.Value;
                        }
                        ...
                        if(kprop == null) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(could not find DictKeyProp \"{strParams[2]}\").");
                            return m.Value;
                        }
                        return kprop.GetValue(subDict.Value.key).ToString();
                    case "DictKeyField":
                        if(!subDict.HasValue) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(DictKeyField tag used on non-BindDict).");
                            return m.Value;
                        }
                        if(strParams.Length < 3) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(not enough params for DictKeyField tag).");
                            return m.Value;
                        }
                        ...
                        if(kfld == null) {
                            AdmiralPlugin._logger.LogWarning($"{errorStr}(could not find DictKeyField \"{strParams[2]}\").");
                            return m.Value;
                        }
                        return kfld.GetValue(subDict.Value.key).ToString();
                }
                AdmiralPlugin._logger.LogWarning($"{errorStr}(unknown tag \"{strParams[1]}\").");
                return m.Value;
            });
        }
        ...
        public void Bind(PropertyInfo prop, ConfigFile cfl, string modName, string categoryName, AutoConfigAttribute attrib, AutoConfigUpdateActionsAttribute eiattr = null, BindSubDictInfo? subDict = null) {
            ...
            if(!subDict.HasValue) {
                if(this.bindings.Exists(x => x.boundProperty == prop)) {
                    AdmiralPlugin._logger.LogError($"{errorStr}this property has already been bound.");
                    return;
                }
                if((attrib.flags & AutoConfigFlags.BindDict) == AutoConfigFlags.BindDict) {
                    if(!prop.PropertyType.GetInterfaces().Any(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IDictionary<,>))) {
                        AdmiralPlugin._logger.LogError($"{errorStr}BindDict flag cannot be used on property types which don't implement IDictionary.");
                        return;
                    }
                    var kTyp = prop.PropertyType.GetGenericArguments()[1];
                    if(attrib.avb != null && attrib.avbType != kTyp) {
                        AdmiralPlugin._logger.LogError($"{errorStr}dict value and AcceptableValue types must match (received {kTyp.Name} and {attrib.avbType.Name}).");
                        return;

                    }
                    if(!TomlTypeConverter.CanConvert(kTyp)) {
                        AdmiralPlugin._logger.LogError($"{errorStr}dict value type cannot be converted by BepInEx.Configuration.TomlTypeConverter (received {kTyp.Name}).");
                        return;
                    }
                    ...
                    var dkeys = (from object k in idict.Keys
                                 select k).ToList();
                    if(dkeys.Count == 0) {
                        AdmiralPlugin._logger.LogError($"{errorStr}BindDict was used on an empty dictionary. All intended keys must be present at time of binding and cannot be added afterwards.");
                    }
            ...
            if(!subDict.HasValue) {
                if(attrib.avb != null && attrib.avbType != prop.PropertyType) {
                    AdmiralPlugin._logger.LogError($"{errorStr}property and AcceptableValue types must match (received {prop.PropertyType.Name} and {attrib.avbType.Name}).");
                    return;
                }
                if(!TomlTypeConverter.CanConvert(prop.PropertyType)) {
                    AdmiralPlugin._logger.LogError($"{errorStr}property type cannot be converted by BepInEx.Configuration.TomlTypeConverter (received {prop.PropertyType.Name}).");
                    return;
                }
            }
            ...
            if(propGetter == null || propSetter == null) {
                AdmiralPlugin._logger.LogError($"{errorStr}property (or IDictionary Item property, if using BindDict flag) must have both a getter and a setter.");
                return;
            }
            ...
            if(!deferForever) {
                ...
                Action<object, EventArgs> lam = (obj, evtArgs) => {
                    newBinding.UpdateProperty(cfe.BoxedValue);
                };
                var pArr = evh.EventHandlerType.GetMethod("Invoke").GetParameters().Select(p => Expression.Parameter(p.ParameterType)).ToArray();
                var h = Expression.Lambda(evh.EventHandlerType, Expression.Call(Expression.Constant(lam), lam.GetType().GetMethod("Invoke"), pArr[0], pArr[1]), pArr).Compile();
                evh.AddEventHandler(cfe, h);
            }
        ...
        public void BindRoO(ConfigEntryBase cfe, PropertyInfo prop, Type propType, string categoryName, string cfgName, string cfgDesc, bool deferForever, bool deferRun, params BaseAutoConfigRoOAttribute[] entryRoOAttributes) {
            ...
            if(entryRoOAttributes.GroupBy(x => x.GetType()).Any(x => x.Count() > 1))
                AdmiralPlugin._logger.LogWarning($"AutoConfigContainer.BindRoO on property {prop.Name} in category {categoryName} has multiple RoO options of the same type");
            ...
            if(!foundModInfo) {
                AdmiralPlugin._logger.LogError($"{errorStr2}could not find mod info. Declaring type must be in an assembly with a BepInPlugin, or have an AutoConfigContainerRoOInfoAttribute on it.");
                return;
            }
            ...
            foreach(BaseAutoConfigRoOAttribute opt in entryRoOAttributes) {
                if((opt.requiredType == typeof(Enum)) ? (!propType.IsEnum) : (propType != opt.requiredType)) {
                    AdmiralPlugin._logger.LogError($"{errorStr2}{opt.GetType().Name} may only be applied to {opt.requiredType.Name} properties (got {propType.Name}).");
                    continue;
                }
